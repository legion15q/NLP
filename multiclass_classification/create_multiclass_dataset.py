...
def main():
    ...
    for index, row in data.iterrows():
        ...
        # print(type(entity_start_pos), entity_start_pos)
        # print(type(entity_end_pos), entity_end_pos)
    ...
    new_df.loc[new_df['tonality'] == -1, 'tonality'] = 2
    new_df.loc[new_df['tonality'] == -2, 'tonality'] = 3
    ...
    X_test, X_val, y_test, y_val = train_test_split(X_test, y_test,
                                                    ...
                                                    random_state=42,
                                                    stratify=y_test)
    print(f"Количество строк в y_train по классам: {np.bincount([item  for item in y_train])}")
    print(f"Количество строк в y_test по классам: {np.bincount([item for item in y_test])}")
    print(f"Количество строк в y_val по классам: {np.bincount([item for item in y_val])}")
